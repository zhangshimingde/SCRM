...
import condition from '@/view/fullpage/menus/xiansuo/cluecondition';
export default {
  name: '',
  directives: {
    TransferDom
  ...
  mounted(){
    if(!this.$route.params.creater||!this.$route.params.date||!this.$route.params.product){
      this.$vux.alert.show({
        title: '友情提示',
        content: '页面传参不完整，请仔细核对！'
      })
    }
  ...
  methods:{
    ...
    getData(isEmpty){
      ...
      this.$http.post("/api/EnergizaSaleClueController/GetClueListMobile",{
        ...
        OpportunityStatus:this.$route.params.status,
        FollowerGUID:"",
        ...
        StandardCreateTime:this.$route.params.date,
        CreatorName:this.$route.params.creater,
        ProductNames:this.$route.params.product,
