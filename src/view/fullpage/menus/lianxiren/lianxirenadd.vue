<template>
  <div class="lianxirenadd">
      ...
      <group id="ha">
        ...
        <cell-box >
          <div class="form-item clearfix" style="" >
            <x-input :title='`<span style="${lianxirenTip}">姓名</span>`'  novalidate :icon-type="lianxirenIcon"  v-model="baseInfo.name" placeholder="必填"  text-align="right"></x-input>
          </div>
        </cell-box>
        <cell-box >
          <div class="form-item clearfix sp" v-if="!disabled" style="" @click="chosekehu=true">
            <x-input :title='`<span style="${commonTip}">公司名称</span>`'  novalidate :icon-type="commonIcon" readonly v-model="baseInfo.company" placeholder="点击选择(必填)"  text-align="right"></x-input>
          </div>
          <div class="form-item clearfix" v-else style="">
            <x-input title='公司名称' readonly v-model="baseInfo.company" placeholder="点击选择(必填)"  text-align="right"></x-input>
          </div>
        </cell-box>
        <cell-box >
          <div class="form-item clearfix " style="" >
            <x-input :title='`<span style="${phoneTip}">手机</span>`'  novalidate :icon-type="phoneIcon" v-model="baseInfo.phone" placeholder="必填" :max="11" text-align="right" @on-blur="onBlur"></x-input>
          </div>
        </cell-box>
        <cell-box>
        ...
        <cell-box>
          <div class="form-item clearfix sp" style="" @click="jcr=true">
            <x-input  :title='`<span style="${jcrTip}">是否决策人</span>`'  novalidate :icon-type="jcrIcon" v-model="baseInfo.jcr" placeholder="点击选择(必填)" readonly  text-align="right"></x-input>
          </div>
        </cell-box>
        <cell-box >
          <div class="form-item clearfix" >
            <x-input  :title='`<span style="${jobsTip}">职务</span>`'  novalidate :icon-type="jobsIcon" v-model="baseInfo.jobs" placeholder="必填"  text-align="right"></x-input>
          </div>
        </cell-box>
        <cell-box>
      ...
      <group style="margin-top:10px">
        ...
        <cell-box>
          <div class="form-item clearfix radio reado" style="padding-right:2px" >
...
export default {
  ...
  created(){
    if(this.$route.params.type=="shangji"){
      ...
      this.$http.get("/api/EnergizaSaleKHInfoController/GetHKInfoByName?KHGUID="+this.$route.params.id)
      .then((res)=>{
        // console.log(res)
        this.disabled=true;
        this.baseInfo.company=res.Data.FullName;
  ...
  methods:{
   ...
   confirms(){
     ...
     if(!this.baseInfo.name){
        this.lianxirenTip= colorStyle
        this.lianxirenIcon=IconStyle
     }else{
       this.lianxirenTip = ''
       this.lianxirenIcon =''
     }
     if(!this.baseInfo.company){
       this.commonTip = colorStyle
       this.commonIcon =IconStyle
     }else{
       this.commonTip = ''
       this.commonIcon =''
     }
     ...
     if(!this.baseInfo.business){
       this.businessTip=colorStyle
       this.businessIcon=IconStyle
     }else{
       this.businessTip = ''
       this.businessIcon =''
     }

      if(!this.baseInfo.jcr){
       this.jcrTip=colorStyle
       this.jcrIcon=IconStyle
     }else{
       this.jcrTip = ''
       this.jcrIcon=''
     }

      if(!this.baseInfo.jobs){
       this.jobsTip=colorStyle
       this.jobsIcon=IconStyle
     }else{
       this.jobsTip = ''
       this.jobsIcon=''
     }
