        ...
        <template v-if="jfList.length>0">
          <li v-for="(item,index) in jfList" :key="index">
            <router-link :to="{name:'jiaofudetail',params:{id:index}}">
              <div class="header">
                <p class="title text-over">{{item.ProjName}}</p>
              ...
              <div class="footer clearfix">
                <div class="left"><i class="iconfont icon-repire" style="font-size:1.4rem;vertical-align:middle;margin-right:2px;color:#2EA0FC"></i>{{item.text}}</div>
                <div class="left text_center text-over">PM:{{item.UserName}}</div>
...
export default {
  ...
  methods:{
    getData(){
      this.loading=true;
      this.$http.post("/api/EnergizaSaleKHInfoController/GetCustomerDelivery",{
      ...
      .then((res)=>{
        ...
        this.panelDataList[0].val=res.Data.DeliverPro;
        this.panelDataList[1].val=res.Data.NewPro;
        this.panelDataList[2].val=res.Data.YSTotal;
        this.panelDataList[3].val=res.Data.EK;
        this.panelDataList[4].val=res.Data.YSPro;
        this.panelDataList[5].val=res.Data.HKYear;
