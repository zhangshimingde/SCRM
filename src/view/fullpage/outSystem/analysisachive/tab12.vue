...
export default {
  ...
  created(){
    ...
    this.getArea();
    this.$cmBus.$on('analysisachiveFresh',el=>{
      // console.log(el)
      this.type=el.area;
      this.date=el.date;
      this.getData();
  ...
  beforeDestroy() {
    // this.$cmBus.$off('analysisachiveFresh');
  },
  components:{
    Selector ,Group,charts,PopupRadio ,numberComma,PopupPicker,InlineLoading
  },
  mixins:[outSystem],
  computed:{
    echartsOptions(){
  ...
  methods:{
    ...
    getData(){
      this.loading=true;
      this.$http.post("/api/EnergizeAllow/GetYJAnalyse",{
