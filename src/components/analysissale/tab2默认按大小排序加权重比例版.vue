  ...
  computed:{
    echartsOptions(){
      return {
        tooltip: {
            trigger: 'item',
            // formatter: "{b}*赢单率30% <br/>预测赢单金额 : {c}万",
            formatter: function (params, ticket, callback) {
              console.log(params)
                let text="";
                switch(params.name){
                  case '需求引导':text="10%";break;
                  case '价值呈现':text="30%";break;
                  case '方案竞争':text="50%";break;
                  case '商务谈判':text="70%";break;
                  case '合同审批':text="90%";break;
                }
                return `${params.name}*赢单率${text} <br/>预测赢单金额 : ${params.value}万`;
            }
        },
        calculable: true,
        series: [
      ...
      .then((res)=>{
        ...
        this.funnelDataAll.map((el,index)=>{
          this.funnelData.push({
              name: el.StageName,
              value:Number((el.SumPredictTradeMoney*(2*index+1)/10).toFixed(2))
          })
