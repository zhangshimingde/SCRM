...
export default {
  name: '',
  created(){
    this.$http.post("/api/EnergizeSaleBulletin/IsShowProductQy").then((res)=>{
  ...
  methods:{
    ...
    getData(){
      ...
      if(this.activeType=="签约"){
        ...
        if(this.activeType=="线索"){url="/api/EnergizeSaleBulletin/GetProductStractureClueAnalyse"}
        else if(this.activeType=="商机"){url="/api/EnergizeSaleBulletin/GetProductStractureOpportunitiesAnalyse"}
        this.$http.post(url,{
    ...
    getArea(){  //获取部门
      ...
      this.$http.post('/api/EnergizeSaleUser/GetRoleAreaOrg')
        .then((res)=>{
          this.fillArea2(res);
      })
      // if(this.activeType=="签约"){
