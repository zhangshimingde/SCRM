  ...
  methods:{
    ...
    pubFinish(param){//发布记录，评论
      ...
      if(this.pubType=="records"){ //发布新的跟进记录
          this.$http.post("/api/EnergizaSaleClueController/SaveFollowUpLog",{
            Content:param,
            OpportunityGUID:this.id,
            SourcePort:"移动端"
          })
          ...
          this.$http.post("/api/AjaxFollowLogController/AddFollowReplyNew",{
            ReplyContent:param,
            ClueFollowRelyId:this.recordId
          })
          .then((res)=>{
            // console.log(res)
            this.$vux.loading.hide()
    ...
    delet(type,item){ //删除当前记录、评论
      ...
      this.$vux.confirm.show({
        ...
        onConfirm () {
          ...
          if(type=='跟进记录'){ //删除记录
              _this.$http.get("/api/EnergizaSaleClueController/DeleteFlowUpLog?FlowUpId="+item.FlowerID)
              .then((res)=>{
